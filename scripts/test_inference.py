...
def main(cfg):
    ...
    # model.cuda()    
    # model.eval()
    # print(f'Loading just eval model into GPU: {model.device}')
    # breakpoint()
    ...
    #     points_action = data_batch["points_action"][0:1].to(model.device)
    #     points_anchor = data_batch["points_anchor"][0:1].to(model.device)
    #     sym_features_action = data_batch["action_symmetry_features"][0:1].to(model.device)
    #     sym_features_anchor = data_batch["anchor_symmetry_features"][0:1].to(model.device)

    #     start_time = time.time()
    #     ans = model(points_action, points_anchor, sym_features_action, sym_features_anchor)
    #     end_time = time.time()
    ...
    # print(f'Average inference time: {total_time / 100}')
    # breakpoint()
    
    
    
    #################################################
    # Eval Train Memory Usage #######################
    #################################################
    ...
    print(f'Loading just train model into GPU: {model.device}')
    # breakpoint()
    ...
    data_batch = next(train_iter)
    print(f'len(data_batch): {len(data_batch)}')
    
    for key in data_batch:
        print(f'key: {key}')
        print(f'data_batch[key].shape: {data_batch[key].shape}')
        
    points_action = data_batch["points_action"].to(model.device)
    points_anchor = data_batch["points_anchor"].to(model.device)
    sym_features_action = data_batch["action_symmetry_features"].to(model.device)
    sym_features_anchor = data_batch["anchor_symmetry_features"].to(model.device)
    
    print(f'Loading model and {points_action.shape[0]} batch data into GPU: {model.device}')
    breakpoint()
