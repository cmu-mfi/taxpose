...
def main(cfg):
    ...
    model = get_training_model(network, cfg, None)

    maybe_load_training_model_weights(model, cfg, None)
    ...
    # Assuming train_dataloader is already defined and loaded as per your code
    train_dataloader = dm.train_dataloader()
    train_iter = iter(train_dataloader)

    # Initialize variables to store stats for action and anchor separately
    stats = {
        "action": {"min": {"length": float('inf'), "width": float('inf'), "height": float('inf'), "density": float('inf')},
                "max": {"length": float('-inf'), "width": float('-inf'), "height": float('-inf'), "density": float('-inf')},
                "sum": {"length": 0, "width": 0, "height": 0, "density": 0},
                "count": 0},
        "anchor": {"min": {"length": float('inf'), "width": float('inf'), "height": float('inf'), "density": float('inf')},
                "max": {"length": float('-inf'), "width": float('-inf'), "height": float('-inf'), "density": float('-inf')},
                "sum": {"length": 0, "width": 0, "height": 0, "density": 0},
                "count": 0}
    }
    ...
    def update_stats(points, stat_type):
        ...
        density = points.shape[1] / volume

        # Update stats
        stats[stat_type]["min"]["length"] = min(torch.min(length).item(), stats[stat_type]["min"]["length"])
        stats[stat_type]["min"]["width"] = min(torch.min(width).item(), stats[stat_type]["min"]["width"])
        stats[stat_type]["min"]["height"] = min(torch.min(height).item(), stats[stat_type]["min"]["height"])
        stats[stat_type]["min"]["density"] = min(torch.min(density).item(), stats[stat_type]["min"]["density"])

        stats[stat_type]["max"]["length"] = max(torch.max(length).item(), stats[stat_type]["max"]["length"])
        stats[stat_type]["max"]["width"] = max(torch.max(width).item(), stats[stat_type]["max"]["width"])
        stats[stat_type]["max"]["height"] = max(torch.max(height).item(), stats[stat_type]["max"]["height"])
        stats[stat_type]["max"]["density"] = max(torch.max(density).item(), stats[stat_type]["max"]["density"])

        stats[stat_type]["sum"]["length"] += torch.sum(length).item()
        stats[stat_type]["sum"]["width"] += torch.sum(width).item()
        stats[stat_type]["sum"]["height"] += torch.sum(height).item()
        stats[stat_type]["sum"]["density"] += torch.sum(density).item()

        stats[stat_type]["count"] += batch_size

    # Iterate through the dataset
    for data_batch in train_iter:
        update_stats(data_batch["points_action"], "action")
        update_stats(data_batch["points_anchor"], "anchor")

    # Calculate mean stats
    mean_stats = {
        "action": {k: v / stats["action"]["count"] for k, v in stats["action"]["sum"].items()},
        "anchor": {k: v / stats["anchor"]["count"] for k, v in stats["anchor"]["sum"].items()}
    }

    print('Action stats:')
    print(f'\tNum points  : {data_batch["points_action"].shape[1]}')
    print(f'\tNum examples: {stats["action"]["count"]}')
    print(f'\tMin length  : {stats["action"]["min"]["length"]:<15.5f} | Max length : {stats["action"]["max"]["length"]:<15.5f} | Mean length : {mean_stats["action"]["length"]:<15.5f}')
    print(f'\tMin width   : {stats["action"]["min"]["width"]:<15.5f} | Max width  : {stats["action"]["max"]["width"]:<15.5f} | Mean width  : {mean_stats["action"]["width"]:<15.5f}')
    print(f'\tMin height  : {stats["action"]["min"]["height"]:<15.5f} | Max height : {stats["action"]["max"]["height"]:<15.5f} | Mean height : {mean_stats["action"]["height"]:<15.5f}')
    print(f'\tMin density : {stats["action"]["min"]["density"]:<15.5f} | Max density: {stats["action"]["max"]["density"]:<15.5f} | Mean density: {mean_stats["action"]["density"]:<15.5f}')
    print('Anchor stats:')
    print(f'\tNum points  : {data_batch["points_anchor"].shape[1]}')
    print(f'\tNum examples: {stats["anchor"]["count"]}')
    print(f'\tMin length  : {stats["anchor"]["min"]["length"]:<15.5f} | Max length : {stats["anchor"]["max"]["length"]:<15.5f} | Mean length : {mean_stats["anchor"]["length"]:<15.5f}')
    print(f'\tMin width   : {stats["anchor"]["min"]["width"]:<15.5f} | Max width  : {stats["anchor"]["max"]["width"]:<15.5f} | Mean width  : {mean_stats["anchor"]["width"]:<15.5f}')
    print(f'\tMin height  : {stats["anchor"]["min"]["height"]:<15.5f} | Max height : {stats["anchor"]["max"]["height"]:<15.5f} | Mean height : {mean_stats["anchor"]["height"]:<15.5f}')
    print(f'\tMin density : {stats["anchor"]["min"]["density"]:<15.5f} | Max density: {stats["anchor"]["max"]["density"]:<15.5f} | Mean density: {mean_stats["anchor"]["density"]:<15.5f}')
