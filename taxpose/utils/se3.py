...
def random_se3(
    ...
    assert rot_sample_method in ["axis_angle", "axis_angle_uniform_z", "quat_uniform", "identity", "random_upright", "random_flat_upright"]
    ...
    if rot_sample_method == "quat_uniform":
        ...
        return Rotate(torch.eye(3).unsqueeze(0).repeat(N, 1, 1), device=device)
    else:
